...
public:
	...
	// tap-then-release SHIFT behavior for pasted shifted characters. Public
	// (like other PORT_CHANGED_MEMBER handlers in MAME) because the ioport
	// configuration code that wires it up via FUNC(...) in pocketc.cpp is
	// not a member of this class. Implemented in pc1360_m.cpp.
	DECLARE_INPUT_CHANGED_MEMBER(shift_chord_changed);

	// QUICKLOAD callback for the "quikload" file-manager slot added in
	// pocketc.cpp's pc1360() -- tokenizes a plain-text .BAS file (native
	...
	// the project for the format/pointer background (including the
	// leading/trailing 0xFF sentinel bytes) this is built on. Public for
	// the same reason shift_chord_changed() is: the device config code
	// that binds it via FUNC(...) in pocketc.cpp isn't a member of this
	// class.
...
private:
	...
	// above -- allocated in machine_start() (pc1360_m.cpp).
